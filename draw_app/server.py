...
def _emit_script(req: BuildRequest, name: str) -> str:
    lines = [
        f'"""Generated by the sketch-to-CAD drawing app — operation list form."""',
        'import sys; sys.path.insert(0, "cad")',
        'from _render import render',
        'import cadquery as cq',
        '',
        '# ---- parameters (edit these to tweak) -----------------------------------------',
        f'TARGET_SIZE = {req.target_size_mm:.3f}   # mm — used when no annotation drives scale',
        '',
        '# ---- operations -----------------------------------------------------------------',
        '# This script encodes the operation list literally. Edit the ops below to tweak.',
        '#',
    ]
    for i, op in enumerate(req.operations):
        d = op.model_dump()
        d.pop("op", None)
        lines.append(f'# Op {i}: {op.op}  params={d}')
    lines.append('')
    lines.append('# For now, the script just re-uses the build_app server-side logic by')
    lines.append('# serializing and POSTing — but here we keep it inline:')
    lines.append('result = None')
    lines.append('# (To recreate this part programmatically, prefer the JSON ops + the server\'s /build')
    lines.append('# endpoint, since reconstructing all op semantics in plain CadQuery is verbose.)')
    lines.append('')
    lines.append(f'# Fallback: if any sketch op produced an extrusion, render it via the helper.')
    lines.append(f'# This is a stub — re-edit the ops above and re-save from the app for now.')
    lines.append(f'result = cq.Workplane("XY").box(20, 20, 10)   # placeholder body')
